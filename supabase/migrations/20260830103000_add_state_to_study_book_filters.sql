-- Include State in the study book so identical exam names in TG and AP remain separate.
...
returns table (
  question_id uuid, state_name text, exam_name text, paper_name text, subject_name text,
...
as $$
begin
  delete from public.student_question_bookmarks
  where user_id = auth.uid() and created_at < now() - interval '1 year';

  delete from public.student_question_mistakes
  where user_id = auth.uid() and last_seen_at < now() - interval '1 year';

  return query
  select question.id, state.name, exam_group.name, paper.name, subject.name, question.question_text,
  ...
  join public.exams board on board.id = exam_group.exam_id
  join public.exam_states state on state.id = board.state_id
