...
export function EmailOtpLoginForm({ nextPath, onUsePassword }: { nextPath: string; onUsePassword: () => void }) {
  ...
  return <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.15em] text-teal-700">Email code sign-in</p><h2 className="mt-2 text-3xl font-black tracking-tight text-slate-950">Sign in with a six-digit code</h2><p className="mt-3 text-sm leading-6 text-slate-600">A secure six-digit code will be sent to your registered email address. A password is not required.</p>
