...
export default async function ExamPagesPage() {
  ...
  const [statesResult, boardsResult, examsResult, faqsResult] = await Promise.all([
    ...
    supabase.from("exam_groups").select("id, exam_id, name, slug, description, seo_title, seo_description").order("display_order"),
  ...
  return <ExamPageFaqManager states={statesResult.data ?? []} boards={(boardsResult.data ?? []).map((item) => ({ id: item.id, stateId: item.state_id, name: item.name }))} exams={(examsResult.data ?? []).map((item) => ({ id: item.id, boardId: item.exam_id, name: item.name, slug: item.slug, description: item.description, seoTitle: item.seo_title, seoDescription: item.seo_description }))} faqs={faqsResult.data ?? []} />;
}
