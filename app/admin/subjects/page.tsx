...
export default async function SubjectsPage({ searchParams }: { searchParams: Promise<{ category?: string; exam?: string; specialization?: string; paper?: string }> }) {
  ...
  return <main><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-teal-700">Exam catalogue</p><h1 className="mt-2 text-3xl font-black">Subjects</h1><p className="mt-2 text-slate-600">Choose the Recruiting Board, Exam, optional Specialisation, and Paper before adding its Subjects.</p></div><SubjectsWorkspace categories={categories} exams={exams} specializations={specializations.map((item) => ({ id: item.id, examId: item.exam_group_id, name: item.name }))} papers={papers} subjects={subjects.map((subject) => ({ id: subject.id, paperId: subject.paper_id, name: subject.name, slug: subject.slug, contentLanguageMode: subject.content_language_mode, isActive: subject.is_active, displayOrder: subject.display_order }))} initialLocation={initialLocation} />{subjectsResult.error && <p className="mt-6 rounded-xl bg-red-50 p-4 text-red-700">{subjectsResult.error.message}</p>}</main>;
}
