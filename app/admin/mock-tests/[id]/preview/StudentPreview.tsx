...
export function StudentPreview({ mockTestId, questions, canEdit, backHref, listReturnTo, initialQuestion }: { mockTestId: string; questions: PreviewQuestion[]; canEdit: boolean; backHref: string; listReturnTo: string; initialQuestion: number }) {
  ...
  return <div className="mt-7">
    ...
    <div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_16rem]">
      ...
      <aside className="rounded-3xl border bg-white p-5 shadow-sm lg:sticky lg:top-36 lg:self-start"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Question navigator</p><p className="mt-2 text-sm leading-6 text-slate-600">Select any question to inspect its student view.</p><div aria-label="Preview question list" className="mt-4 grid max-h-72 grid-cols-5 gap-2 overflow-y-auto overscroll-contain pr-1 sm:max-h-80 lg:max-h-[calc(100vh-18rem)]">{questions.map((question, questionIndex) => <button key={question.id} type="button" onClick={() => setIndex(questionIndex)} aria-label={`Preview question ${questionIndex + 1}`} aria-current={questionIndex === index ? "step" : undefined} className={`grid aspect-square place-items-center rounded-lg text-xs font-black ${questionIndex === index ? "bg-slate-950 text-white ring-2 ring-teal-300 ring-offset-2" : "bg-slate-100 text-slate-700 hover:bg-teal-50"}`}>{questionIndex + 1}</button>)}</div><p className="mt-5 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-600">This is an administrator-only preview. Students cannot see answer keys or explanations before submitting their test.</p></aside>
    </div>
  </div>;
}
