...
export default async function MockTestStudentPreviewPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ question?: string | string[] }> }) {
  ...
  const questionsHref = `/admin/mock-tests/${id}/questions`;
  const canEdit = testResult.data.status === "draft" && (attemptCountResult.count ?? 0) === 0;
  return <main><div className="flex flex-wrap items-center justify-between gap-3"><Link href={questionsHref} className="text-sm font-semibold text-teal-700 hover:underline">← Back to Questions</Link><Link href={`/admin/mock-tests/${id}/edit`} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold text-slate-800 hover:bg-slate-50">Mock Test Settings</Link></div><p className="mt-5 text-xs font-black uppercase tracking-[0.14em] text-teal-700">Quality check</p><h1 className="mt-2 text-3xl font-black text-slate-950">Student Preview</h1><p className="mt-2 max-w-3xl text-slate-600">Review exactly how students will read each question, including images and mobile-friendly navigation. The answer key and explanations below are visible only to administrators.</p>{!canEdit && <p className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold leading-6 text-amber-950">This mock test is no longer editable because it is published or has student attempts. The preview remains available for quality review.</p>}<StudentPreview mockTestId={id} questions={questions} canEdit={canEdit} backHref={questionsHref} initialQuestion={initialQuestion} /></main>;
}
